...
async def b2b_public_products_batch(payload: dict[str, Any], request: Request, x_service_key: str = Header(...)):
    store = get_store(request)
    products = []
...
async def b2b_public_product(product_id: str, request: Request, x_service_key: str = Header(...)):
    store = get_store(request)
    try:
...
async def b2b_public_similar(product_id: str, request: Request, x_service_key: str = Header(...), limit: int = 10):
    store = get_store(request)
    try:
...
async def b2b_public_sku(sku_id: str, request: Request, x_service_key: str = Header(...)):
    store = get_store(request)
    try:
